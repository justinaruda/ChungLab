# -*- coding: utf-8 -*-
"""
Created on Sun Jul 18 08:03:33 2021
...
anot_df = pd.read_table(annotation_file, header=None, names=get_header(annotation_file))

anot_df['Region'].replace(regex="_",value="",inplace=True)

anot_df = anot_df[~anot_df['Region'].str.startswith('#')]

# for idx, feature in anot_df:
#     anot_df['Region'].iloc[idx] = feature['Region'].replace("_","")

anot_io = io.StringIO()

anot_df.to_csv(anot_io, sep="\t", line_terminator="\n", index=False, header=False, quoting=csv.QUOTE_NONE)

anot_io.seek(0)

if annotation_file.endswith('.gtf'):
    annots = HTSeq.GFF_Reader(anot_io,end_included=True)
    annotation_type = 'gtf'
if annotation_file.endswith('.bed'):
    annots = HTSeq.BED_Reader(anot_io)
    annotation_type = 'bed'

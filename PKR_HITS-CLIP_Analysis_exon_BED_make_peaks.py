# -*- coding: utf-8 -*-
"""
Created on Tue Jul 20 12:35:21 2021
...
short_options = "b:a:o:p:q:x:svz"
long_options = ["bam=", "bed=", "annotation_file=", "output=", "peak_file=", "mapq=", "bed_output", "strict", "verbose", "keep_zeros"]
...
if peak_file.endswith('.bed'):
    ...
    peaks = HTSeq.GFF_Reader(peak_io,end_included=True)
...
if annotation_file.endswith('.gtf'):
    annots = HTSeq.GFF_Reader(annot_io,end_included=True)
    annotation_type = 'gtf'
...
#        input_read_names = set()
        ...
        for clip_tag in clip_tags:
            ...
            if out_bed:
                if clip_type == 'bam':
                    bed_chrom.append(clip_tag.iv.chrom)
                    bed_start.append(clip_tag.iv.start)
                    bed_end.append(clip_tag.iv.end)
                    bed_strand.append(clip_tag.iv.strand)
                    bed_score.append('.')
                if clip_type == 'bed':
                    bed_chrom.append(clip_tag.contig)
                    bed_start.append(clip_tag.start)
                    bed_end.append(clip_tag.end)
                    bed_score.append(clip_tag.score)
                    bed_strand.append(clip_tag.strand)
                bed_name.append(clip_tag.name)
...
final_gene_count_df['Annotation'] = gene_list
...
for gene in gene_list:
    ...
    if len(gene_count_array) > 0:
        final_gene_count_df.loc[(final_gene_count_df['Annotation']==gene),'Peak_Count'] = len(gene_count_array)
        final_gene_count_df.loc[(final_gene_count_df['Annotation']==gene),'Average_Tags'] = np.average(gene_count_array) #FC
        final_gene_count_df.loc[(final_gene_count_df['Annotation']==gene),'Total_Tags'] = np.sum(gene_count_array)
        final_gene_count_df.loc[(final_gene_count_df['Annotation']==gene),'Peak_Tags'] = '['+', '.join(map(str,[(str(tup[0]),tup[1]) for tup in gene_counts[gene]]))+']'

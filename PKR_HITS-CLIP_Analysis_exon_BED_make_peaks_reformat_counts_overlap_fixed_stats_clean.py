# -*- coding: utf-8 -*-
"""
Created on Tue Aug 10 16:21:37 2021
...
if peak_file.endswith('.bed'):
    ...
    peak_type = 'bed'
elif peak_file.endswith('.gtf'):
    peaks = HTSeq.GFF_Reader(peak_io,end_included=end_included)
    peak_type = 'gtf'
else:
    print('Peak file must be in .bed or .gtf format.')
    sys.exit()
...
if stats:
    ...
    if not no_peaks:
        k = 0
        checked = set()
        end_increment = 1 if peak_type == 'gtf' and not end_included else 0
        for idx,peak in peaks_df.iterrows():
            ...
            for tag in tags:
                if tag.name not in checked:
                    k+=1
        total_assessed_tags = k
    else:
        total_assessed_tags = total_tags
...
#        input_reads = input_bam_reader.fetch(region=chrom+':'+str(start)+'-'+str(end))
        ...
        if not check_label(label):
            set_label(label,[(HTSeq.GenomicInterval(chrom,start,end),clip_count)])
        else:
            append_label(label,(HTSeq.GenomicInterval(chrom,start,end),clip_count))
...
if stats:
    stats_series = pd.Series({"Total_Library_Tags:":total_tags,"Total_Assessed_Tags:":total_assessed_tags,"Unique_Assigned_Tags:":unique_assigned_tags,"Multiple_Assigned_Tags:":multiple_assigned_tags,"Total_Assigned_Tags:":final_gene_count_df['Total_Tags'].sum()})

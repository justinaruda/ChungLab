...
rule get_intervals:
	input:
		"../{sample}.pool.tag.uniq.peak.sig.bed"
	output:
		"bed_intervals/{sample}.pool.tag.uniq.peak.sig.{iv}nt.{genome}.slop.bed"
	params:
...
rule get_random_interval:
	...
	shell:
		"bedtools random -l {wildcards.iv} -n 8000 -g /home/hchunglab/data/justin/RNA*/genome/prim*/grch38_primary_assembly_measles.genome > {output}"

rule get_fastas:
	input:
		"bed_intervals/{sample}.pool.tag.uniq.peak.sig.{iv}nt.{genome}.slop.bed"
	output:
		"fastas/{sample}.pool.tag.uniq.peak.sig.{iv}nt.{genome}.slop.fasta"
	params:
		genome=get_genome
	shell:
...
rule fold:
	input:
		"fastas/{sample}.pool.tag.uniq.peak.sig.{iv}nt.{genome}.slop.fasta"
	output:
		"RNAfold/{sample}.pool.tag.uniq.peak.sig.{iv}nt.{genome}.fold"
	threads: 8
	shell:
...
def combine_inputs(wildcards):
	a = expand("RNAfold/{sample}.pool.tag.uniq.peak.sig.{iv}nt.{genome}.fold",sample=samples,iv=ivs,genome=genomes)

# -*- coding: utf-8 -*-
"""
Created on Tue Aug  3 14:32:36 2021
...
if stats:
    ...
    for idx,peak in peaks_df.iterrows():
        ...
        if clip_type == 'bam' and peak['Region'] not in bam_chroms:
            continue
        if clip_type == 'bed':
            tags = clip_reader.fetch(peak['Region'],int(peak['Start']),int(peak['End']),parser=pysam.asBed())
        if clip_type == 'bam':
            tags = clip_reader.fetch(region="{0}:{1}-{2}".format(peak['Region'],str(peak['Start']),str(peak['End'])))
        for tag in tags:

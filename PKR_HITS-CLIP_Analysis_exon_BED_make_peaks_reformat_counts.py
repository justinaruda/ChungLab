...
if stats:
    ...
    i = 0
    for peak in peaks_df.iterrows():
        tags = clip_reader.fetch(peak['Region'],peak['Start'],peak['End'])
        for tag in tags:
            i+=1
    total_peak_tags = i
...
#        input_read_names = set()
        ...
        for clip_tag in clip_tags:
            ...
            if strict or stats:
                in_checked_tags = (tag_name in checked_tags)
                if strict and in_checked_tags:
                    continue
                if stats and not in_checked_tags:
                    unique_assigned_tags += 1
                if stats and in_checked_tags:
